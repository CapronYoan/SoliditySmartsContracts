...
contract DVNFT is ERC721, ERC165, Ownable, Pausable, ReentrancyGuard {
    ...
    constructor (string memory name_, string memory symbol_, string memory baseURI_, address _adminwallets, address _oracle) {
        ...
        _beforeTokenTransfer(address(0), adminWallets.getTreasuryWallet(), 0);
        _balances[adminWallets.getTreasuryWallet()] += 1;
        _owners[0] = adminWallets.getTreasuryWallet();
        _tokenClass[0] = THE_UNKNOWN;
        _tokenClassSupplyCurrent[THE_UNKNOWN] = 1;
        
        _beforeTokenTransfer(address(0), adminWallets.getWallet1(), 1);
        _beforeTokenTransfer(address(0), adminWallets.getWallet2(), 2);
        _beforeTokenTransfer(address(0), adminWallets.getWallet2(), 3);

        // Minting three kings for Diversity, AssetBender and Marketing.
        _balances[adminWallets.getWallet1()] += 1;
        _balances[adminWallets.getWallet2()] += 1;
        _balances[adminWallets.getWallet3()] += 1;

        _owners[1] = adminWallets.getWallet1();
        _owners[2] = adminWallets.getWallet2();
        _owners[3] = adminWallets.getWallet3();
        _owners[4] = adminWallets.getWallet4();
    ...
    function airdropToTreasury(uint256[][] memory treasuryRandom) external onlyOwner {
        address TreasuryWallet = adminWallets.getTreasuryWallet();
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _beforeTokenTransfer(address(0), TreasuryWallet, tokenIds[i]);
            _balances[TreasuryWallet] += 1;
            _owners[tokenIds[i]] = TreasuryWallet;
            _tokenClass[tokenIds[i]] = classIds[i];
            _tokenClassSupplyCurrent[classIds[i]] += 1;
            emit Transfer(address(0), TreasuryWallet, tokenIds[i]);
        }
